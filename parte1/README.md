...
Después miramos qué versiones tenemos instaladas en las carpetas habituales:
...
Consultamos las variables del usuario y del sistema por separado. En el usuario no hay un `JAVA_HOME`, pero en el sistema ya apunta a `C:\Program Files\Zulu\zulu-17\`. Además, el `PATH` coloca `zulu-17\bin` antes que las otras versiones. Las variables guardadas están bien. Abrimos otra ventana de PowerShell para comprobar que usa esos valores.
...
Esta vez `JAVA_HOME` apunta a `C:\Program Files\Zulu\zulu-17\`, y tanto `java` como `javac` muestran la versión 17.0.20.1. Los dos ejecutables están en `C:\Program Files\Zulu\zulu-17\bin`. Así comprobamos que esta ventana de PowerShell ya utiliza JDK 17. No hace falta reinstalarlo ni volver a cambiar las variables del sistema.
...
Instalamos Almond 0.14.5 indicando la versión Scala 2.12.21:
...
Visual Studio Code ya está instalado en el equipo. Abrimos una ventana nueva y consultamos Help > About para comprobar su versión: 1.137.0, instalado para nuestro usuario.
...
En una PowerShell nueva, desde `C:\clase3\workspace`, comprobamos si podemos ejecutar sbt:
...
Descargamos `sbt-1.13.0.msi` desde la página oficial. Windows muestra un aviso de SmartScreen con editor desconocido. Revisamos el nombre del archivo y seguimos con la instalación.
...
Abrimos una PowerShell nueva y ejecutamos `sbt --version` desde `C:\clase3\workspace`. Esta vez el comando responde con `sbt runner version: 1.13.0` y vuelve a permitirnos escribir comandos sin mostrar errores. El mensaje indica la versión del programa que arranca sbt. La versión de sbt de cada proyecto se indica en `project/build.properties`.
...
Esta vez `Project's Java` muestra 17.0.20.1 y el proyecto `scala-vscode` aparece con Scala 2.12.21 y todos sus indicadores verdes. Metals sigue funcionando con Java 21, pero el proyecto utiliza JDK 17.
...
Entramos en Plugins y buscamos Scala en Marketplace. Seleccionamos el complemento de JetBrains s.r.o. En su ficha aparece la versión 2024.3.35.
...
Creamos un proyecto llamado `scala-intellij` dentro de `C:\clase3\workspace`. Seleccionamos sbt para compilar y ejecutar el proyecto, Zulu JDK 17 y Scala 2.12.21. Dejamos desmarcadas las opciones de crear un repositorio Git y añadir código de ejemplo.

![Configuración del nuevo proyecto Scala en IntelliJ](../images/35-configuracion-proyecto-intellij.png)

Al abrir el proyecto, comprobamos que `build.sbt` contiene `ThisBuild / scalaVersion := "2.12.21"` y `name := "scala-intellij"`. La opción `ThisBuild` establece la versión de Scala para todo el proyecto. El archivo `project/build.properties` fija `sbt.version = 1.10.3`.
